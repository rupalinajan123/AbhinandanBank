       ...
       $request->validate([
        ...
        if ($status['success']) 
        {
            ...
            //  print_r($encryptedAccNumber);exit;
                ...
                $branchEmail = $branchEmails[$request->branch] ?? 'support@abhinandanbank.net';
                $mailData = [
                    ...
                    'email_body' => "
                        ...
                        <head>
                             <style>
                                ...
                                .details { margin: 20px 0; padding: 10px; background: #fff; border: 1px solid #ddd; border-radius: 5px; }
                        ...
                        <body>
                            ...
                            <div class='details'>
                                <div><span class='label'>Customer Name:</span> <span class='value'>{$request['customer_name']}</span></div>
                                <div><span class='label'>Subject:</span> <span class='value'>{$request['subject']}</span></div>
                                <div><span class='label'>Category:</span> <span class='value'>{$request['category']}</span></div>
                                <div><span class='label'>Complaint Details:</span> <span class='value'>{$request['cdetails']}</span></div>
                                <div><span class='label'>Branch:</span> <span class='value'>{$request['branch']}</span></div>
                                <div><span class='label'>Address:</span> <span class='value'>{$request['address']}</span></div>
                                <div><span class='label'>Email:</span> <span class='value'>{$request['email']}</span></div>
                                <div><span class='label'>Mobile:</span> <span class='value'>{$request['mobile']}</span></div>
                                <div><span class='label'>Account Head:</span> <span class='value'>{$request['acc_head']}</span></div>
                            </div>
                            <div class='footer'>
                ...
                // Use the `html` method to send the email content as HTML
                try {
                    Mail::send([], [], function ($message) use ($branchEmail, $mailData) {
                        $message->to($branchEmail)
                                // ->cc('ceo@abhinandanbank.net', 'support@abhinandanbank.net') // Add CC email address here
                                ->subject($mailData['subject'])
                                ->setBody($mailData['email_body'], 'text/html'); // Set the email body as HTML
                    });
